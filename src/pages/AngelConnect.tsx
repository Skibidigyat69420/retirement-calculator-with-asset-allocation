...
export const AngelConnect = () => {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Tabs */}
      <div className="flex border-b border-border gap-6 overflow-x-auto whitespace-nowrap">
        <button
          onClick={() => setActiveTab('connect')}
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'connect' ? 'text-ink border-b-2 border-brass' : 'text-ink-soft hover:text-ink'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'guide' ? 'text-ink border-b-2 border-brass' : 'text-ink-soft hover:text-ink'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'holdings' ? 'text-ink border-b-2 border-brass' : 'text-ink-soft hover:text-ink'
          }`}
        >
        ...
        <button
          ...
          className={`pb-3 text-sm font-semibold transition-all relative ${
            activeTab === 'scripts' ? 'text-ink border-b-2 border-brass' : 'text-ink-soft hover:text-ink'
          }`}
        >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-lg font-serif text-ink">SmartAPI Connection Parameters</h3>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-ink flex items-center gap-1 hover:text-ink underline"
              >
            ...
            {statusMessage && (
              <div
                className={`p-3.5 rounded-xl text-sm flex items-start gap-2.5 ${
                  statusMessage.type === 'success'
                    ? 'bg-accent-softer text-accent-strong border border-accent-soft'
              ...
              >
                {statusMessage.type === 'success' ? (
                  <CheckCircle2 size={18} className="text-accent-strong shrink-0 mt-0.5" />
                ) : (
            ...
            <form onSubmit={handleConnect} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    aria-label="API Key"
                    className="w-full px-3.5 py-2.5 bg-raised border border-border rounded-xl text-sm font-mono focus:outline-none focus:border-navy"
                    required
                  />
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-ink-soft mb-1">
                    Angel One Client Code (User ID) <span className="text-negative">*</span>
                  </label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-ink-soft mb-1">
                    MPIN / Password <span className="text-negative">*</span>
                  </label>
                  <input
                    type="password"
                    ...
                    className="w-full px-3.5 py-2.5 bg-raised border border-border rounded-xl text-sm focus:outline-none focus:border-navy"
                    required
                  />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="text-[11px] text-ink-soft hover:text-ink underline flex items-center gap-1"
                    >
              ...
              {creds.totpSecret && (
                <div className="p-3 bg-surface rounded-xl border border-border flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-[11px] uppercase tracking-wider text-ink-soft">Live Generated TOTP</div>
                      <div className="text-base font-mono font-bold text-ink tracking-widest">
                        {activeTotp || '------'}
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xs text-ink-soft">{totpCountdown}s refresh</div>
                    <div className="w-20 bg-sunken h-1.5 rounded-full overflow-hidden mt-1">
                      <div
                        className="bg-accent-softer0 h-full transition-all duration-1000"
              ...
              <div className="border border-border rounded-xl p-4 space-y-3 bg-cream/40">
                <button
                  type="button"
                  onClick={() => setShowAdvancedNetwork(!showAdvancedNetwork)}
                  className="w-full flex items-center justify-between text-xs font-semibold text-ink uppercase tracking-wider"
                >
                  <span className="flex items-center gap-2">
                    <Sliders size={14} className="text-muted" /> System Network Headers (Auto-Detected)
                  </span>
                ...
                {showAdvancedNetwork && (
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 text-xs">
                    <div>
                      <label className="block text-ink-soft mb-1">Public IP (ClientPublicIP)</label>
                      <input
                        type="text"
                        value={creds.publicIp}
                        onChange={(e) => setCreds({ ...creds, publicIp: e.target.value })}
                        aria-label="Public IP"
                        className="w-full px-2.5 py-1.5 bg-raised border border-border rounded-lg font-mono text-xs"
                      />
                    </div>
                    <div>
                      <label className="block text-ink-soft mb-1">Local IP (ClientLocalIP)</label>
                      <input
                        type="text"
                        value={creds.localIp}
                    ...
                    <div>
                      <label className="block text-ink-soft mb-1">MAC Address (MACAddress)</label>
                      <input
                        type="text"
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                {session && (
                  <Button type="button" variant="outline" onClick={handleDisconnect} className="py-3 text-negative">
                    Disconnect
                  </Button>
                )}
              </div>
              ...
              {session && (
                <div className="pt-4 border-t border-border">
                  ...
                  {historicalTest.count > 0 && (
                    <p className="text-xs text-accent-strong mt-2 text-center">
          ...
          <div className="space-y-6">
            <Card className="bg-deep text-raised relative overflow-hidden">
              ...
              <h3 className="text-lg font-serif text-ink mb-2">Network & Identity</h3>
              <p className="text-xs text-faint mb-4">
                Angel One SmartAPI matches these headers with your registered developer app.
              </p>

              <div className="space-y-3 text-xs font-mono">
                <div className="bg-raised/10 p-2.5 rounded-lg flex items-center justify-between">
                  <div>
                    <span className="text-raised block text-[10px]">PUBLIC IP</span>
                    <span className="text-ink font-semibold">{creds.publicIp}</span>
                  </div>
                  <button
                    onClick={() => handleCopy(creds.publicIp, 'pubIp')}
                    className="text-faint hover:text-ink p-1 rounded focus:outline-none focus:ring-2 focus:ring-accent/50"
                    title="Copy Public IP"
                    aria-label="Copy Public IP"
                    type="button"
                  >
                    {copiedKey === 'pubIp' ? <Check size={14} className="text-accent" /> : <Copy size={14} />}
                  </button>
                </div>

                <div className="bg-raised/10 p-2.5 rounded-lg flex items-center justify-between">
                  <div>
                    <span className="text-raised block text-[10px]">LOCAL IP</span>
                    <span className="text-ink font-semibold">{creds.localIp}</span>
                  </div>
                  <button
                    ...
                    type="button"
                  >
                    {copiedKey === 'locIp' ? <Check size={14} className="text-accent" /> : <Copy size={14} />}
                  </button>
                </div>

                <div className="bg-raised/10 p-2.5 rounded-lg flex items-center justify-between">
                  <div>
                    <span className="text-raised block text-[10px]">MAC ADDRESS</span>
                    <span className="text-ink font-semibold">{creds.macAddress}</span>
                  </div>
                  <button
                    ...
                    type="button"
                  >
                    {copiedKey === 'mac' ? <Check size={14} className="text-accent" /> : <Copy size={14} />}
                  </button>
                </div>
              </div>
            </Card>
            ...
            {session && (
              <Card className="space-y-4">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-surface flex items-center justify-center text-ink font-bold">
                    <User size={20} />
                  </div>
                  <div>
                    ...
                    <div className="text-xs text-ink-soft">Angel One Account Active</div>
                  </div>
                </div>
                ...
                {funds && (
                  <div className="grid grid-cols-2 gap-2 pt-2 border-t border-border-subtle text-xs">
                    <div className="p-2.5 bg-surface rounded-lg">
                      <span className="text-ink-soft block text-[10px]">AVAILABLE CASH</span>
                      <span className="font-semibold text-ink">{formatCurrency(funds.availablecash)}</span>
                    </div>
                    <div className="p-2.5 bg-surface rounded-lg">
                      <span className="text-ink-soft block text-[10px]">TOTAL MARGIN</span>
                      <span className="font-semibold text-ink">{formatCurrency(funds.availablemargin)}</span>
                    </div>
                  </div>
                )}
      ...
      {activeTab === 'guide' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="space-y-5">
            <h3 className="text-lg font-serif text-ink flex items-center gap-2">
              <Key size={20} className="text-muted" /> Step 1: Create App on Angel One Portal
            </h3>

            <div className="space-y-3 text-xs text-ink-soft">
              <div className="p-3 bg-surface rounded-xl border border-border">
                <div className="font-semibold text-ink text-sm mb-1">1. Visit the Developer Portal</div>
                <p>
                  Go to{' '}
                  <a
              ...
              <div className="p-3 bg-surface rounded-xl border border-border">
                <div className="font-semibold text-ink text-sm mb-1">2. Click "Create App"</div>
                <p className="mb-2">Fill in the creation form with these recommended parameters:</p>
                <ul className="space-y-1.5 font-mono">
                  <li className="flex justify-between items-center bg-raised p-2 rounded border border-border">
                    <span>App Name:</span>
                    <span className="text-ink font-semibold">SoundThesisWealth</span>
                  </li>
                  <li className="flex justify-between items-center bg-raised p-2 rounded border border-border">
                    <span>API Type:</span>
                    <span className="text-ink font-semibold">Trading API</span>
                  </li>
                  <li className="flex justify-between items-center bg-raised p-2 rounded border border-border">
                    <span>Redirect URL:</span>
                    <span className="text-ink font-semibold">http://localhost:5173/</span>
                  </li>
                  <li className="flex justify-between items-center bg-raised p-2 rounded border border-border">
              ...
              <div className="p-3 bg-surface rounded-xl border border-border">
                <div className="font-semibold text-ink text-sm mb-1">3. Copy your API Key</div>
          ...
          <Card className="space-y-5">
            <h3 className="text-lg font-serif text-ink flex items-center gap-2">
              <ShieldCheck size={20} className="text-muted" /> Step 2: Enable Automated TOTP
            </h3>

            <div className="space-y-3 text-xs text-ink-soft">
              <div className="p-3 bg-surface rounded-xl border border-border">
                <div className="font-semibold text-ink text-sm mb-1">1. Open Enable TOTP Page</div>
                <p>
                  Visit{' '}
                  <a
                    href="https://smartapi.angelone.in/enable-totp"
              ...
              <div className="p-3 bg-surface rounded-xl border border-border">
                <div className="font-semibold text-ink text-sm mb-1">2. Copy the QR / Base32 Secret</div>
                <p>
                  You will see a QR code with a text code underneath (e.g.{' '}
                  <code className="bg-sunken px-1 py-0.5 rounded">JBSWY3DPEHPK3PXP...</code>). Copy this Secret string!
                </p>
              </div>

              <div className="p-3 bg-accent-softer rounded-xl border border-accent-soft text-accent-strong">
                <div className="font-semibold text-accent-strong text-sm mb-1">3. Automated 30s Live OTP</div>
                <p>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card className="bg-raised">
              <div className="text-xs text-ink-soft font-medium">Total Live Portfolio Value</div>
              <div className="text-2xl font-serif font-bold text-ink mt-1">{formatCurrency(totalHoldingsValue)}</div>
              <div className="text-xs text-ink-soft mt-1">{holdings.length} Positions</div>
            </Card>

            <Card className="bg-raised">
              <div className="text-xs text-ink-soft font-medium">Total Unrealized P&L</div>
              <div className={`text-2xl font-serif font-bold mt-1 ${totalPnL >= 0 ? 'text-accent-strong' : 'text-negative'}`}>
            ...
            <Card className="bg-accent-softer/10 border-accent-soft/30 flex flex-col justify-between">
              <div>
                <div className="text-xs text-ink font-semibold">Sync with Wealth Planner</div>
          ...
          <Card className="overflow-x-auto">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base font-serif text-ink">Live Angel One Demat Holdings</h3>
              {session && (
                <Button
                  variant="outline"
                  onClick={() => loadPortfolioData(session, creds)}
            ...
            {holdings.length === 0 ? (
              ...
              <table className="w-full min-w-[540px] text-left text-xs">
                <thead>
                  <tr className="border-b border-border text-ink-soft uppercase tracking-wider text-[10px]">
                    ...
                    <tr key={idx} className="hover:bg-surface/80 transition-colors">
                      <td className="py-3 font-semibold text-ink flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-ink-soft font-normal">{h.exchange}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-3 text-right font-mono font-semibold text-ink">{formatCurrency(h.ltp)}</td>
                      <td className="py-3 text-right font-mono font-semibold">{formatCurrency(h.totalHoldingValue)}</td>
                      <td className={`py-3 text-right font-mono font-semibold ${h.pnl >= 0 ? 'text-accent-strong' : 'text-negative'}`}>
      ...
      {activeTab === 'scripts' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-serif text-ink flex items-center gap-2">
                <Terminal size={18} className="text-muted" /> Node.js Standalone Script
              </h3>
          ...
          <Card className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-base font-serif text-ink flex items-center gap-2">
                <Terminal size={18} className="text-muted" /> Python Connector Script
              </h3>

...
export const PlanHealthScoreCard = ({ health }: PlanHealthScoreCardProps) => {
  ...
  if (health.overallScore < 70) {
    ...
    scoreColor = 'text-zinc-700 bg-zinc-50 border-zinc-300';
    badgeVariant = 'warning';
  }

  return (
    <Card className="p-6 bg-white border border-zinc-200/90 shadow-2xs space-y-5">
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-100 pb-4">
        <div className="flex items-start gap-4">
          ...
          >
            <span className="text-2xl font-bold font-sans leading-none">{health.overallScore}</span>
            <span className="text-[10px] font-bold uppercase tracking-wider mt-0.5">/ 100</span>
          </div>

          <div className="space-y-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-lg font-bold text-zinc-950 tracking-tight font-sans">
            ...
            <p className="text-xs text-zinc-600 leading-relaxed max-w-2xl">{health.headline}</p>
          </div>
        </div>
        ...
        <Button
          ...
          className="text-xs h-8 px-3 gap-1.5 self-start sm:self-center shrink-0 border-zinc-300 text-zinc-700 hover:bg-zinc-100"
        >
      ...
      {health.keyAttentionItems.length > 0 && (
        <div className="p-3.5 rounded-xl bg-rose-50/50 border border-rose-200 text-xs text-rose-950 space-y-1.5">
          <div className="flex items-center gap-1.5 font-bold text-rose-950 uppercase tracking-wider text-[11px]">
            <AlertTriangle size={14} className="text-rose-600 shrink-0" />
            {health.keyAttentionItems.length} Key Item{health.keyAttentionItems.length > 1 ? 's' : ''} Requiring Action:
          </div>
        ...
        {health.components.map((comp) => {
          ...
          return (
            <div
              ...
              className="p-2.5 rounded-xl border border-zinc-200/80 bg-zinc-50/60 hover:bg-zinc-100/90 hover:border-zinc-300 cursor-pointer transition-all space-y-1.5"
            >
      ...
      {expanded && (
        <div className="border-t border-zinc-100 pt-4 space-y-3 animate-in fade-in duration-200">
          <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-500 flex items-center gap-1.5">
            <Info size={13} className="text-zinc-700" />
            Plan Health Methodology & Area Breakdown
          </h4>
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-zinc-50/80 transition-colors ${
                        selectedComp?.id === comp.id ? 'bg-zinc-100/80' : ''
                      }`}
                    >
                      <td className="py-2.5 font-bold text-zinc-950">{comp.name}</td>
                      <td className="py-2.5 text-right font-sans font-bold text-zinc-950 pr-3">
                        {comp.score}/100
                      </td>
                      <td className="py-2.5">{badge}</td>
                      <td className="py-2.5 pr-3 text-zinc-600 leading-relaxed">{comp.reason}</td>
                      <td className="py-2.5 text-zinc-800 font-medium leading-relaxed">

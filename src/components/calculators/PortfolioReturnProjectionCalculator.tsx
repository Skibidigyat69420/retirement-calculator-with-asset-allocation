  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-200/80 pb-4">
        ...
        {/* Preset Selector */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-zinc-500">Preset:</span>
          <select
            aria-label="Preset portfolio allocation template"
            onChange={(e) => handleApplyPreset(e.target.value)}
            className="text-xs font-semibold bg-white border border-zinc-200 rounded-lg px-3 py-1.5 text-zinc-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl border border-zinc-200 bg-white space-y-1 shadow-xs">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs font-semibold text-zinc-500">Nominal p.a.</span>
          </div>
        ...
        <div className="p-4 rounded-xl border border-zinc-200 bg-white space-y-1 shadow-xs">
          ...
          <p className="text-xs text-zinc-500">Total future rupee valuation</p>
        </div>
        ...
        <div className="p-4 rounded-xl border border-zinc-200 bg-white space-y-1 shadow-xs">
          ...
          <p className="text-xs text-zinc-500">Deflated at {inflationRate}% annual inflation</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="p-5 border border-zinc-200/90 shadow-sm space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-zinc-800 border-b border-zinc-100 pb-2 flex items-center gap-1.5">
        ...
        <div className="lg:col-span-8 space-y-4">
          <Card className="p-5 border border-zinc-200/90 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded font-mono font-bold ${
                    isWeight100 ? 'bg-emerald-100 text-emerald-800' : 'bg-zinc-100 text-amber-800'
                  }`}
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-xs h-7 px-2.5 gap-1 bg-zinc-900 text-white hover:bg-zinc-800 border-none"
                >
                  <Plus size={13} />
                  Add Class
                </Button>
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {assetClasses.map((ac, idx) => {
                    ...
                    return (
                      <tr key={ac.id} className="hover:bg-zinc-50/70 transition-colors">
                        {/* Name */}
                        <td className="py-2.5 pr-2">
                          <input
                            ...
                            className="w-full font-semibold text-zinc-800 bg-transparent border-b border-transparent hover:border-zinc-300 focus:border-indigo-500 focus:outline-none transition-colors"
                          />
                        </td>
                        ...
                        <td className="py-2.5 px-2 text-right">
                          <input
                            ...
                            min={0}
                            max={100}
                            className="w-16 text-right font-mono font-bold text-zinc-900 bg-zinc-50 border border-zinc-200 rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                        </td>
                        ...
                        <td className="py-2.5 px-2 text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <input
                              ...
                              step={0.5}
                              className="w-16 text-right font-mono text-zinc-900 bg-zinc-50 border border-zinc-200 rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                            />
                        ...
                        <td className="py-2.5 px-2">
                          <select
                            ...
                            className="w-full font-semibold text-zinc-700 bg-zinc-50 border border-zinc-200 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          >
            ...
            <div className="p-3 bg-zinc-50 rounded-lg text-xs text-zinc-600 flex items-start gap-2">
              <Globe size={15} className="text-indigo-600 mt-0.5 shrink-0" />
              <div>
                <span className="font-semibold text-zinc-800">Currency Compounding Note:</span> Foreign assets (e.g. US Equities in USD) automatically compound at their local return plus foreign currency appreciation against the Indian Rupee: <code className="text-indigo-700 font-mono">(1 + r_local) × (1 + r_fx) - 1</code>. Real returns are further deflated by domestic inflation ({inflationRate}%).
              </div>
            </div>
          </Card>
        </div>
      ...
      <Card className="p-6 border border-zinc-200/90 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-zinc-100 pb-3">
          ...
          {/* Chart Mode Toggle */}
          <div className="flex items-center gap-1.5 p-1 bg-zinc-100 rounded-lg self-start sm:self-center">
            <button
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-100 font-mono">
              {projection.snapshots
                ...
                .map((s) => (
                  <tr key={s.year} className="hover:bg-zinc-50/80 transition-colors">
                    <td className="py-2.5 font-sans font-bold text-zinc-800">
                      {s.year === 0 ? 'Initial (Yr 0)' : `Year ${s.year}`}
                    </td>
                    <td className="py-2.5 text-right text-zinc-600">{formatCurrency(s.totalInvested)}</td>
                    <td className="py-2.5 text-right font-bold text-zinc-900">{formatCurrency(s.nominalValue)}</td>
                    <td className="py-2.5 text-right font-semibold text-emerald-700">{formatCurrency(s.realValue)}</td>
                    <td className="py-2.5 text-right text-zinc-700">+{formatCurrency(s.nominalGain)}</td>

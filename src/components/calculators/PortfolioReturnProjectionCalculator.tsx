  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border/80 pb-4">
        ...
        {/* Preset Selector */}
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-muted">Preset:</span>
          <select
            aria-label="Preset portfolio allocation template"
            onChange={(e) => handleApplyPreset(e.target.value)}
            className="text-xs font-semibold bg-surface border border-border rounded-lg px-3 py-1.5 text-ink-soft focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl border border-border bg-surface space-y-1 shadow-xs">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-xs font-semibold text-muted">Nominal p.a.</span>
          </div>
          <p className="text-xs font-mono font-medium text-positive">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-4 space-y-4">
          <Card className="p-5 border border-border/90 shadow-sm space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-ink-soft border-b border-border pb-2 flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`p-2 rounded-lg border font-medium transition-all ${
                    rebalanceAnnually
                      ...
                      : 'bg-surface text-ink-soft border-border hover:bg-sunken'
                  }`}
                >
                  Annual Rebalance
                </button>
        ...
        <div className="lg:col-span-8 space-y-4">
          <Card className="p-5 border border-border/90 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-border pb-3">
              <div className="flex items-center gap-2">
                ...
                <span
                  className={`text-xs px-2 py-0.5 rounded font-mono font-bold ${
                    isWeight100 ? 'bg-emerald-100 text-positive' : 'bg-sunken text-warning'
                  }`}
                >
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  className="text-xs h-7 px-2.5 gap-1 bg-sunken text-ink hover:bg-raised border-none"
                >
                  <Plus size={13} />
                  Add Class
                </Button>
              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-border">
                  {assetClasses.map((ac, idx) => {
                    ...
                    return (
                      <tr key={ac.id} className="hover:bg-sunken/70 transition-colors">
                        {/* Name */}
                        <td className="py-2.5 pr-2">
                          <input
                            ...
                            className="w-full font-semibold text-ink-soft bg-transparent border-b border-transparent hover:border-border-strong focus:border-indigo-500 focus:outline-none transition-colors"
                          />
                        </td>
                        ...
                        <td className="py-2.5 px-2 text-right">
                          <input
                            ...
                            min={0}
                            max={100}
                            className="w-16 text-right font-mono font-bold text-ink bg-sunken border border-border rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                        </td>
                        ...
                        <td className="py-2.5 px-2 text-right">
                          <div className="flex items-center justify-end gap-0.5">
                            <input
                              ...
                              step={0.5}
                              className="w-16 text-right font-mono text-ink bg-sunken border border-border rounded px-1.5 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                            />
                        ...
                        <td className="py-2.5 px-2">
                          <select
                            ...
                            className="w-full font-semibold text-ink-soft bg-sunken border border-border rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                          >
                        ...
                        <td className="py-2.5 px-2 text-right font-mono font-bold text-ink">
                          {eff ? `${eff.effectiveInrReturn.toFixed(1)}%` : '-'}
                        </td>

                        {/* Real Return */}
                        <td className="py-2.5 px-2 text-right font-mono font-semibold text-positive">
                        ...
                        <td className="py-2.5 pl-2 text-center">
                          {assetClasses.length > 1 && (
                            <button
                              ...
                              className="text-faint hover:text-negative transition-colors p-1"
            ...
            <div className="p-3 bg-sunken rounded-lg text-xs text-muted flex items-start gap-2">
              <Globe size={15} className="text-indigo-600 mt-0.5 shrink-0" />
              <div>
                <span className="font-semibold text-ink-soft">Currency Compounding Note:</span> Foreign assets (e.g. US Equities in USD) automatically compound at their local return plus foreign currency appreciation against the Indian Rupee: <code className="text-indigo-700 font-mono">(1 + r_local) × (1 + r_fx) - 1</code>. Real returns are further deflated by domestic inflation ({inflationRate}%).
              </div>
            </div>
          </Card>
        </div>
      ...
      <Card className="p-6 border border-border/90 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3">
          ...
          {/* Chart Mode Toggle */}
          <div className="flex items-center gap-1.5 p-1 bg-sunken rounded-lg self-start sm:self-center">
            <button
          ...
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-border font-mono">
              {projection.snapshots
                ...
                .map((s) => (
                  <tr key={s.year} className="hover:bg-sunken/80 transition-colors">
                    <td className="py-2.5 font-sans font-bold text-ink-soft">
                      {s.year === 0 ? 'Initial (Yr 0)' : `Year ${s.year}`}
                    </td>
                    <td className="py-2.5 text-right text-muted">{formatCurrency(s.totalInvested)}</td>
                    <td className="py-2.5 text-right font-bold text-ink">{formatCurrency(s.nominalValue)}</td>
                    <td className="py-2.5 text-right font-semibold text-positive">{formatCurrency(s.realValue)}</td>
                    <td className="py-2.5 text-right text-ink-soft">+{formatCurrency(s.nominalGain)}</td>
                    <td className="py-2.5 text-right font-medium text-positive">

...
export const GoalCalculator = () => {
  ...
  const fundingRows: { label: string; value: string }[] = [
    { label: "Target (today's value)", value: formatCurrency(result.target) },
    { label: 'Inflation-adjusted target', value: formatCurrency(result.futureValue) },
    { label: 'Invest lumpsum today', value: formatCurrency(result.requiredLumpsum) },
    { label: 'OR monthly SIP', value: formatCurrency(result.requiredSIP) },
    { label: `OR step-up SIP (growing ${stepUp}%/yr)`, value: formatCurrency(result.requiredSIPWithStepUp) },
  ];
  ...
  return (
    <CalculatorShell
      ...
      inputs={
        <>
          <div className="space-y-2.5 pb-4 border-b border-border-subtle">
            <Select
            ...
            {activePlanGoal ? (
              <>
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  >
                    <RefreshCw size={13} strokeWidth={1.6} className="mr-1" /> Revert
                  </Button>
                  <Button
                    type="button"
                ...
                <div className="flex justify-end pt-1">
                  {confirmDelete ? (
                    <div className="flex items-center gap-2 bg-negative-soft border border-negative/25 px-2.5 py-1.5 rounded-md">
                      ...
                      <button
                        ...
                        className="px-1.5 py-0.5 bg-raised hover:bg-sunken text-ink-soft border border-border rounded-sm text-[11px] transition-colors cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="text-xs text-negative hover:bg-negative-soft px-2 py-1 rounded-sm transition-colors flex items-center gap-1 font-medium cursor-pointer"
                      title="Delete this goal from the plan"
                    >
                      <Trash2 size={13} strokeWidth={1.6} /> Delete Goal
                    </button>
                  )}
                </div>
              </>
              ...
              <Button
                ...
                variant="primary"
              >
                <Sparkles size={14} strokeWidth={1.6} className="mr-1.5" /> Add to Plan Goals
              </Button>
            )}
          </div>
        </>
      ...
      results={
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <MetricCard
            ...
            icon={<Coins size={18} strokeWidth={1.6} />}
            variant="gold"
          />
          <MetricCard
    ...
    >
      <div className="mt-5 rounded-lg border border-border bg-raised">
        <div className="px-5 pt-4 pb-3 border-b border-border-subtle flex items-center justify-between">
          <div className="eyebrow">Goal Funding Options</div>
          {activePlanGoal && (
            <Badge tone="positive">Connected to {activePlanGoal.name}</Badge>
          )}
        </div>
        <div className="p-5">
      ...
      <div className="mt-5 rounded-lg border border-border bg-raised">
        <div className="px-5 pt-4 pb-3 border-b border-border-subtle">
          <div className="eyebrow">Funding Trajectory</div>
        </div>
        <div className="p-5">
          ...
          <div className="h-64" role="img" aria-label={`Area chart of SIP corpus growth over ${years} years versus the inflation-adjusted goal of ${formatCurrencyCompact(result.futureValue)}. The corpus reaches the target at year ${years}.`}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={fundingCurve} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                ...
                <XAxis dataKey="year" tick={{ fontSize: 11, fill: theme.axisLabel }} axisLine={false} tickLine={false} />
                <YAxis
                ...
                <ReferenceLine
                  ...
                  strokeDasharray="6 4"
                  label={{ value: `Goal ${formatCurrencyCompact(result.futureValue)}`, position: 'insideTopRight', fill: 'var(--color-negative)', fontSize: 11 }}
                />
                <Area
                  type="monotone"
                  dataKey="corpus"

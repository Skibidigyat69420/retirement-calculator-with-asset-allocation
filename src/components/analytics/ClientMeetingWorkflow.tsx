...
const STAGES: ClientMeetingStageInfo[] = [
  ...
  },
  {
    ...
    description: 'Formulate strategic policy allocation, cash surplus waterfall, MVO frontier optimization, and transition plan.',
    checklist: [
      ...
      { id: 'm3-rebalance', label: 'Mean-Variance Frontier (MVO) Optimization', completed: false, route: '/mvo' },
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-4">
          ...
          <div className="flex items-center gap-3 self-start sm:self-center">
            <div className="text-right">
              ...
              <span className="text-sm font-bold font-mono text-slate-900">{progressPercent}% Completed</span>
            </div>
          ...
          {STAGES.map((s) => {
            ...
            return (
              ...
              >
                ...
                <p className={`text-[11px] mt-1 line-clamp-2 leading-snug ${isSelected ? 'text-slate-300' : 'text-slate-500'}`}>
                  {s.description}
                </p>
              </button>
            );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            {activeStageId < 4 && (
              <Button size="sm" onClick={handleAdvanceStage} className="bg-slate-900 text-white hover:bg-slate-800 text-xs">
              ...
              return (
                ...
                >
                  <label className="flex items-center gap-3 cursor-pointer flex-1 min-w-0">
                    ...
                    />
                    <span className={`text-xs font-semibold truncate ${isChecked ? 'line-through text-slate-400' : 'text-slate-800'}`}>
                      {item.label}
                    </span>
                  </label>
                  ...
                  <Link
                    to={item.route}
                    className="text-[11px] text-slate-600 hover:text-slate-900 font-semibold flex items-center gap-1 shrink-0 px-2 py-1 rounded hover:bg-slate-100"
                  >
          ...
          <div className="pt-3 border-t border-slate-100">
            ...
            <div className="flex flex-wrap gap-2">
              {activeStage.keyOutputs.map((out) => (
                <span key={out} className="px-2.5 py-1 bg-slate-100 text-slate-800 rounded-lg text-xs font-semibold flex items-center gap-1.5">
        ...
        <Card className="space-y-4 flex flex-col justify-between">
          <div className="space-y-3">
            ...
            <textarea
              ...
              className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-800 focus:outline-none focus:border-slate-900 resize-none font-sans leading-relaxed"
            />
          </div>

          <div className="space-y-3 pt-2">
            <Button size="sm" onClick={handleSaveNotes} className="w-full bg-slate-900 text-white hover:bg-slate-800 text-xs">
            ...
            <div className="p-3 bg-amber-50/70 border border-amber-200/80 rounded-xl text-[11px] text-amber-900 space-y-1">
              ...
              <p className="leading-snug text-amber-900/90">

    ...
    <Card className="p-6 border border-border/90 shadow-sm space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {(['equity', 'debt', 'gold', 'realestate', 'liquid'] as AssetCategory[]).map((cat) => {
          ...
          return (
            <div key={cat} className="p-3 rounded-xl border border-border/80 bg-surface/50 space-y-1">
              <span className="text-[10px] uppercase font-bold text-muted block truncate">
                {ASSET_LABELS[cat] || cat}
              </span>
              <div className="flex items-baseline justify-between">
                <span className="text-sm font-bold font-mono text-ink">{actual.toFixed(0)}%</span>
                <span className="text-[11px] text-muted">Plan: {target}%</span>
              </div>
              <div className="text-[11px] font-mono font-semibold">
                Drift:{' '}
                <span className={Math.abs(drift) > 5 ? (drift > 0 ? 'text-ink-soft' : 'text-negative') : 'text-accent-strong'}>
      ...
      {Math.abs(equityDrift) > 5 && (
        <div className="p-3.5 rounded-xl bg-surface/70 border border-border/80 text-xs text-ink flex items-start gap-2.5">
          <AlertTriangle size={15} className="text-ink-soft mt-0.5 shrink-0" />
          <div className="space-y-0.5">
            <span className="font-bold text-ink">Calculated Portfolio Drift Consequence:</span>
